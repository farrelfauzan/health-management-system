...
| `SATUSEHAT_KYC_PRIVATE_KEY` / `SATUSEHAT_KYC_PUBLIC_KEY` | generated by us, per deployment | [SATUSEHAT KYC keys](#satusehat-kyc-keys) |
| `SATUSEHAT_KYC_SERVER_PUBLIC_KEY`                    | Kemenkes (owner unknown, see below) | [SATUSEHAT KYC keys](#satusehat-kyc-keys) |
...
Three PEMs, set together or not at all; an empty string counts as unset, and
a partial or unparsable set **disables KYC with a reason** rather than
stopping the API (P24-T14, FR-KYC-01). They are not encryption keys in the
sense of the section above: nothing at rest is sealed under them, so losing
one costs a restart with a new one, not data.

- `SATUSEHAT_KYC_PRIVATE_KEY` / `SATUSEHAT_KYC_PUBLIC_KEY` — the
  deployment's own RSA-2048 pair. **We generate it**; nothing about it is
  registered with Kemenkes. The public half is sent *inside* every encrypted
  `generate-url` request (`public_key`), which is how the platform seals its
  answer for our private half. Generate with
  `openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048 -out kyc.key`
  and `openssl pkey -in kyc.key -pubout -out kyc.pub`; PKCS#8 and SPKI PEM.
  The API refuses a public key that does not belong to the private key
  (`KYC_KEY_PAIR_MISMATCH`).
- `SATUSEHAT_KYC_SERVER_PUBLIC_KEY` — the **platform's** RSA public key, used
  to wrap the per-request AES key. It is per platform (staging ≠ production).
  **Where it is obtained is not documented anywhere we can reach**: the public
  KYC documentation page redirected to the docs index on 2026-09-13
  (`docs/ops/klinik-bidan-sandbox-spike.md` §3), and no key was issued with
  our sandbox credentials. Community clients ship it as a file handed over
  during onboarding. The owner on the Kemenkes side — SATUSEHAT partnership
  desk or the facility's onboarding contact — is **unknown**; ask through the
  facility's SATUSEHAT registration channel and record the answer here.

Rotation: generate a new pair, set both, restart. There is no overlap window
to manage — each request carries the public half, so the platform has nothing
cached to invalidate. A new server key from Kemenkes is set the same way.
