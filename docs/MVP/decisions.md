...
- **Where this record lives:** P20-T04 asked for `docs/decisions/`, which does not exist in this repository and never has. There are two decision files, and their numbering already collides: `docs/post-mvp/decisions.md` opens by saying the MVP file "ends at D-021", which stopped being true when D-022 through D-027 were added here, so each of those six numbers now names two different decisions — and the post-MVP file additionally uses D-026 twice on its own. This record continues the P20 cluster (D-024, D-025, D-026) here as D-027, the post-MVP preamble is corrected to say where its own numbering starts and why, and renumbering the six duplicates is left to the docs chore rather than smuggled into an unrelated PR. P22-T01's D-033 keeps the number it was filed under, in `docs/post-mvp/decisions.md`.
