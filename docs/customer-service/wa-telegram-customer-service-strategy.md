...
> **Amended at `PCS-T12` (D-EMB-01): the default is now hosted.** `EMBEDDING_PROVIDER` selects Together AI (`intfloat/multilingual-e5-large-instruct`) or local Ollama, and Together is the default — a clinic should not need a GPU host to run its own FAQ. The replacement model is deliberately **1024-wide and multilingual**, which is what keeps the rest of this paragraph true: the `vector(1024)` column is unchanged, and cross-lingual ID↔EN retrieval survives the move. Together's English-only embedders would have fit the column and given that up with no error at all.
>
> Two consequences the sentence above does not cover. **The second processor is now real**: every ingested chunk and every retrieval query crosses to a third party, which is exactly what the original decision avoided, so it needs a UU PDP register entry before production. Patient documents are `GENERAL` and never embedded, so no patient record text crosses — staff knowledge-base text and question text do. And **switching provider requires a full re-ingest**: the stamps do their job, so the failure mode is zero results rather than wrong ones, but a corpus embedded by the other provider is invisible until it is re-embedded.
...
10. `PCS-T10` `WahaAdapter` parity implementation + adapter contract test suite (the same fixture conversations must pass through both). **Done.** D-CS-01 stops being a claim and becomes something CI checks. Five decisions carry it. **The contract suite is the deliverable, not the adapter.** Two adapter files cannot demonstrate parity by inspection — they *look* different, because the wire formats are — so one table of fixture conversations, described by what the customer did rather than by what either bridge sends, runs through both implementations and asserts what the conversation layer above them receives. A third adapter (the Cloud API endgame) adds a column and inherits every assertion. **Chat ids are canonicalised, and this is the change that makes the failover real rather than nominal.** GOWA addresses a customer as `628…@s.whatsapp.net` and WAHA as `628…@c.us`; storing whichever bridge happened to deliver a message would make `(channel, externalChatId)` — the key a conversation, its transcript, and every `ChannelPatientLink` hang off — depend on the container in front of the clinic, so switching would silently orphan every live conversation and every proven verification. That is a migration, not a fallback. Both normalizers now reduce to one stored form and both adapters render it back on the way out; GOWA's spelling was chosen as canonical for the boring reason that it is already in the database, so `PCS-T10` needs no migration. **Session health and QR pairing became their own port.** `PCS-T09` put them on the concrete GOWA class for want of a second implementation; now there is one. They are deliberately *not* on the messaging port, because a paired device that can silently log out is a property of a self-hosted bridge and the official Cloud API has neither a QR code nor a session to lose — folding them together would make the endgame implementation the one that cannot satisfy the port. **One webhook route, one secret, both bridges.** The URL is registered inside the running container, so a failover that also required editing it would be a failover with an extra step to forget under pressure; the guard reads either signature header, since GOWA and WAHA agree on HMAC-SHA256 and disagree only on the header name. The body is parsed against a permissive superset and *read* according to `WA_GATEWAY_KIND` rather than sniffed, because guessing would let a malformed GOWA body be re-read as a WAHA one and half-understood. **Both adapters are constructed on every boot and one is bound**, which costs nothing — neither opens a connection in its constructor — and means a change that breaks the unselected adapter fails immediately instead of on the day of a failover, which is the worst possible day to find out. The [runbook](../ops/whatsapp-number-warmup.md) gains the switch procedure and states its one real cost plainly: the two bridges keep incompatible session state, so failing over is a **re-pair**, a few minutes of downtime — and no lost conversations. ⚠️ Same standing caveat as `PCS-T09`: no SIM and no paired device, so neither bridge has answered a real message. The suite runs against mocked HTTP, which is what lets it run in CI at all.
11. `PCS-T11` Load/abuse testing, rate limits verified, UU PDP notice copy reviewed, staged rollout (announce the WA number only after 2 clean weeks on Telegram). **Done**, and it closes Phase CS-3 by turning §8's controls from a list of intentions into things that are enforced, counted, and signed off. Writing the abuse suite is what found the gaps, which is the argument for having written it: **two of §8.3's named controls did not exist**. There was no clinic-wide daily LLM budget — only a per-chat hourly limit, which bounds what one customer costs and does nothing about a hundred chats, and arranging a hundred chats on a public channel costs nothing. And enumeration was only ever counted *per chat*, so opening a second chat reset the quota for free; §8.3 asks for repeated failed challenges against the same **registered number**, which is a different question and the one that makes walking the registry visible. Both now exist, and both are deliberately shaped: the budget counts provider calls rather than tokens, because tokens are only known after a provider answers and a token budget can therefore only stop the call *after* the one that broke it; and enumeration **flags rather than blocks**, because the same shape is produced by a family sharing one registered number, so a person is what tells the two cases apart. The flag also changes nothing the customer can see — the booking still completes, the reply is still the standard confirmation, and the note goes to the transcript rather than the wire, because an attacker learning they were noticed is the one signal worth withholding. **The suite also found two live holes in the shared prompt-injection patterns**, which means they were open on the in-app chatbot too: `enter debug mode` walked past a rule written for `developer mode`, and `you are now a helpful database assistant` matched nothing, because the impersonation list only covered *clinical* roles. Both are fixed for both channels — the third time a customer-service test has found a defect in shared safety patterns, which is the case for pointing an adversarial suite at guards written for a different surface. **The UU PDP notice was revised, not merely reviewed**: it disclosed that the assistant is automated, what the data is for, and where sensitive data is completed, and it did not disclose that messages are processed by a **third-party AI provider** — the one fact a customer cannot possibly infer, since they believe they are texting a clinic — nor the 90-day retention, nor any route to exercise rights. All three were added while keeping it short, because a notice long enough to scroll is a notice nobody reads and an unread notice is not consent. Counsel sign-off remains a **blocker** on the checklist rather than a caveat, since [privacy-notice.id.md](../legal/privacy-notice.id.md) is still marked `PLACEHOLDER`. §8.4's five metrics ship as an endpoint and a card, derived from rows the channel already writes rather than from counters this code increments — a counter is a second source of truth that drifts, resets on deploy, and cannot answer a question nobody thought to count — and every rate is rendered next to the counts behind it, because a handoff rate of 1.0 is alarming over a hundred conversations and meaningless over one. The gate itself is the [go/no-go checklist](../ops/whatsapp-rollout-go-no-go.md), with numeric thresholds, a rollback rehearsal, and a decision line that takes three names. ⚠️ The standing caveat is unchanged and now matters most: **no SIM, no paired device, and no live traffic**, so "two clean weeks on Telegram" has not started. Every control is proven against the code; none is proven against a customer.
...
| D-EMB-01 | Embeddings move to a **hosted provider by default** (Together AI, `intfloat/multilingual-e5-large-instruct`, 1024 dims), selected by `EMBEDDING_PROVIDER`; local Ollama `bge-m3` stays a first-class supported option. **Amends D-CS-06 and Phase 15 §5.4** | Operator decision at `PCS-T12`: a clinic should not need a GPU host to run its own FAQ. The multilingual 1024-wide model is what keeps both properties the local decision bought — the `vector(1024)` column needs no migration, and cross-lingual ID↔EN retrieval survives. **The cost is real and is the reason the switch is explicit**: the hosted embedder is a data processor that sees every ingested chunk and every retrieval query, so it needs a UU PDP register entry before production. Patient documents are `GENERAL` and never embedded, so no patient record text crosses; staff knowledge-base text and question text do |
