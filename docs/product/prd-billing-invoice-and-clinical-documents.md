...
| **The send itself** | **Ships** (`P16-T26`). `DocumentDeliveryWorker` polls the outbox and claims due rows under a lease with `FOR UPDATE SKIP LOCKED` (FR-E4-13), then `DeliverySendService` re-checks the invoice and consent at send time (FR-E4-10), locks the pinned snapshot for an attachment or mints the token for a link, and sends through `WhatsappGatewayService.sendDocument` or `MailService` with the Indonesian-first caption that names the clinic, the patient, the number, the total and the date and nothing clinical (FR-E4-15). Transient failures back off exponentially to `DELIVERY_MAX_ATTEMPTS`, then FAILED with *Retry*; a rejected address fails at once. Claimed rows go out one after another through the same pacing chain replies use, in small batches (FR-E4-17); the daily cap is a rolling 24-hour count and ships unset (§7.4.5.1). `GET /deliveries/:id` reads one delivery. **Not yet:** a "not on WhatsApp" check before the first attempt (US-E4-04's second case) — neither bridge exposes one on the port, so such a number exhausts its retries instead. |
| **Scheduled delivery** | **Ships** (`P16-T38`). `sendAt` on the send request parks the rows until then (FR-E4-09) — one predicate wider in the worker's claim, nothing else new; `POST /deliveries/:id/cancel` and `/reschedule` while it waits. Consent, the number and the invoice are re-checked when it fires, and a withdrawn consent or a voided bill cancels the send with the reason on the timeline (FR-E4-10). |
