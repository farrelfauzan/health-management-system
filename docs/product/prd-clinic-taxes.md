...
- R17. **Jasa medis is a ledger written from paid invoices** (P27-T06, module `clinician-fee`, admin tab Tagihan › Jasa medis).
  - **Rules.** A `ClinicianFeeRule` names exactly one target — a tariff or a `ServiceTariffCategory` — and optionally one clinician (`DoctorProfile`, doctor or midwife). Mode PERCENT (0–100% of what the patient paid for the line) or FIXED (rupiah per unit × quantity, capped at the line). Effective dates are clinic-local days, both ends inclusive, end optional. **Most specific wins**: clinician + tariff → tariff → clinician + category → category, among the rules in force on the payment day. Two live rules for the same target and clinician may not overlap (409 `CLINICIAN_FEE_RULE_OVERLAP`), so one rule per level is ever in force. Target and clinician are fixed once created; editing changes only later payments; deleting is soft.
  - **Attribution.** An invoice's lines belong to the encounter's clinician, or the admitting doctor for an inpatient bill; a walk-in lab bill has no clinician and earns no fee. A medication line has neither a tariff nor a category, so it never earns a fee. There is no per-line "procedure performer" in the schema — `Procedure` records who *wrote* it and the invoice line is not linked to the procedure — so every line uses the encounter clinician.
  - **Ledger.** `ClinicianFeeEntry` rows are written **inside the payment transaction** (ACCRUAL) and inside the void transaction (REVERSAL, every amount negated, in the void month). Each stores the line amount, the clinician's gross fee, the clinic share (they always add up, CHECK), a snapshot of the rule's mode and value, and the clinic-local `YYYY-MM` period of the payment or void (`CLINIC_TIMEZONE`). `(invoice_item_id, kind)` is unique and inserts skip duplicates, so paying or voiding twice never double-writes. A PERCENT fee is rounded half up to a whole rupiah; money moves as integer cents.
  - **Voiding a paid invoice.** PAID is terminal on main (`INVOICE_STATUS_TRANSITIONS`; refunds out of scope), so today the reversal path never fires from the product. The void route already calls it, and it is a no-op for an unpaid invoice; allowing PAID → VOID (a refund decision) is what makes it live.
  - **Statement.** `GET /clinician-fees/statements?period=` lists every clinician's month; `GET /clinician-fees/statements/:doctorId?period=` the lines and totals; `…/export` the CSV (text cells escaped against formula injection, amounts left numeric so a reversal's minus still sums; audited as EXPORT). Keys `clinician-fee.read:any` / `clinician-fee.write:any`, ADMIN only (SUPER_ADMIN through the catalog); a clinician reading their own statement is a follow-up.
  - **Open for T07.** `grossFee` is the clinician's share (Rp90,000 of a Rp150,000 line at 60%) per the ticket's acceptance. PMK 168/2023 practice for a doctor paid through a facility may put the PPh 21 base at what the patient paid before the facility's share; `lineAmount` is stored on every entry so T07 can use either once §6 is answered.
