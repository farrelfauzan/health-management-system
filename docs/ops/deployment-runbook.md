...
- `S3_REGION`, `S3_BUCKET`, `S3_ENDPOINT`, `S3_FORCE_PATH_STYLE`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY`, `S3_REQUEST_TIMEOUT_MS`, `S3_SIGNED_URL_EXPIRES_IN_SECONDS`, `S3_MAX_UPLOAD_SIZE_BYTES`, `S3_ALLOWED_MIME_TYPES` — object storage. `S3_MAX_UPLOAD_SIZE_BYTES` and `S3_ALLOWED_MIME_TYPES` bound browser-direct uploads too, and `S3_SIGNED_URL_EXPIRES_IN_SECONDS` is how long a client has to finish one — a 300s default is generous for a document and tight for a video, so raise it deliberately rather than discovering it as a failed upload.
- **Bucket CORS is a prerequisite for browser-direct uploads.** A presigned PUT is issued by the API but sent by the browser, so the bucket must allow `PUT` from the web origin with `Content-Type` and `Content-Length` in `AllowedHeaders`, and expose `ETag`. Without it the upload fails in the browser with a CORS error while the API logs nothing at all — the signature was fine, the request never arrived. The same applies to MinIO in self-hosted deployments.
...
- **Seed region master data**: the same `pnpm db:seed` also loads `prisma/wilayah.sql` since `P19-T10`, about 8 seconds for 91,599 province, regency, district and village rows. It is idempotent and only touches a row whose name or parent changed. An environment upgraded to that release without a seed run answers every region lookup empty, and a patient create carrying an address chain fails validation. See [region-master-data.md](region-master-data.md).
