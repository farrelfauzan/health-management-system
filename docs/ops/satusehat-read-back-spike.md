...
| **P21-T08** | `GET /Practitioner/:id` returns **no `gender` and no `birthDate`**, and its NIK is **masked to the last three digits** (`*************ddd`, re-probed 13 Sep 2026). The confirmation is the name, plus a check that the visible digits agree with ours, which can prove a mismatch but never a match.                                                                                                                           |
