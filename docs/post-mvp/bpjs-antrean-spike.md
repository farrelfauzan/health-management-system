...
**There is now an instrument for this** (`P14-T06`). Setting `BPJS_PROTOCOL_CAPTURE_DIR` appends every BPJS exchange — outbound and inbound, accepted and rejected — as redacted NDJSON, with the request timestamp kept because the response key derives from it. It is off by default and must be turned off again after the session. The checklist that drives it, and the rules for converting a capture into committed fixtures, are in [bpjs-antrean-uat.md](./bpjs-antrean-uat.md). None of this makes a fixture exist: the list below is still empty until BPJS is on the other end of the line.
...
- **No hand-written fixtures.** A fixture is evidence. One assembled from a reading of the spec is a restatement of the hypothesis wearing evidence's clothes, and it will pass every test written against it. **Unamended and unqualified** — `P14-T06` shipped the instrument that records real ones, which is the opposite of writing them, and the fixture set is still empty.
