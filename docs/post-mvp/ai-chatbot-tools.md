...
| `get_appointment_load` | `AppointmentManagementService.listSessionsCalendar` | `appointment.session.read:any` | **No** — per-session capacity and booked counts; no attendee rows |
...
**The `get_appointment_load` row was corrected when `P15-T18` implemented it, and it is the same class of error the two pharmacy rows had.** The table said `appointment.read:any`; `listSessionsCalendar` asserts `AppointmentSession:read`. `ADMIN` holds both, so the wrong declaration would have worked today and broken silently the moment the grants diverged — and invariant 2 is that a tool is the same door as the REST route, which means declaring the permission that route actually opens rather than the one whose name reads closest.

**`get_daily_cashier_report` is also the one tool whose backing service takes no `CurrentUser`.** `CashierReportService.getDailyReport` resolves no scope, because a cash-drawer report has no ownership dimension — there is one drawer and one clinic day — and its REST route is gated entirely by `@Auth([{ action: 'read', subject: 'Invoice' }])`. The registry's `requiredPermission` therefore *is* that gate reproduced, and the tool is still the same door. Recorded because a reviewer seeing an unused `user` parameter should be able to tell a deliberate exception from an oversight, and should check that the service has not since grown a scope.
...
9. `P15-T19` Intent accuracy (§4.7): tool descriptions written to §4.7.1 (including Indonesian trigger phrasing and explicit *do not use for*), `.describe()` on every argument field, the **unsourced-claim guard** (§4.7.2) in `SafetyPolicyService` with `unsourced_claim` tagging, and the bilingual eval set with its five metrics **run per `AiProviderKind`**. The eval is what says whether a given provider config may enable tools at all — a model that cannot select them is refused the surface rather than allowed to answer from training data. Lands with `P15-T05`, not after `P15-T06`: the pharmacy tools are enough to measure selection, and measuring before patient tools exist is the point. **Code done; the measurement is not, and the distinction is recorded in [ai-chatbot-tool-selection-eval.md](./ai-chatbot-tool-selection-eval.md) §1 rather than glossed.** The guard, the descriptions, the 25-case bilingual set and the scorer are built and unit-tested; the results table is empty because running it needs live vendor credentials and spends real tokens, so it is a script rather than a CI job. **The gate is therefore still open** — no config has been cleared to run `AI_CHAT_TOOLS_ENABLED=true` on evidence. The guard's §4.7.2 scope was narrowed on implementation and the narrowing is deliberate: **names are not matched**, because a pattern for "asserts a patient's name" is a pattern for capitalised words and would flag every mention of a drug or a guideline — an unusable guard gets disabled; and **bare dates and clock times are not matched**, because "klinik buka pukul 08.00" is a static fact the assistant is supposed to answer without a lookup, so only a time asserted as *the reader's own schedule* is caught. What remains — counts of clinic entities, stock levels, currency, and own-schedule times — is where the §4.7.2 failure actually shows up. The guard's precondition is what makes it sound rather than its patterns: it fires only when the catalogue was non-empty **and** the model requested nothing from it, both known before the lookups run, since what matters is whether the model asked rather than whether it then succeeded. Passing no sourcing at all leaves it inert, so the Phase 13 call shape cannot start refusing replies.

**Admin channel (§2.1.2) — independent of the patient tools, and the cheapest track here:**

10. `P15-T17` `ChatChannel.ADMIN`: enum migration, `chatChannelSchema` in shared-types, an admin system prompt (operational framing, no clinical-safety copy written for a clinician), patient-channel output-guard treatment per §2.3, and RBAC binding so only `ADMIN` / `SUPER_ADMIN` may open the channel. Blocks `T18`. **Done.** The migration is `ALTER TYPE ... ADD VALUE`, additive and one-way — Postgres cannot drop an enum value — but it rewrites no row and invalidates no index, and the CI drift gate was reproduced against the migrated database with no `ChatChannel` difference reported. Four decisions beyond the enum. **The channel is bound at session creation, not at message time**: the channel decides the system prompt, the context policy and the tool catalogue, so a session opened in the wrong channel is mis-shaped from its first turn, and refusing it once beats filtering its consequences on every message — the other two channels stay deliberately unbound, since `chat.session.create:own` already opens them and a doctor with their own patient record has a legitimate reason to open a patient session about their own care. **The check is not the tool gate**, and both exist: `ChatToolRegistry` re-derives the channel/role agreement on every offer and every dispatch, so an `ADMIN` holding a doctor-channel session is offered zero tools even though this check is what stops that session existing. **The admin prompt carries no clinical-safety copy at all** — an administrator is not a patient who might mistake the assistant for a clinician, and telling one to "consult a healthcare professional" about a queue length is noise that teaches them to skim the safety lines that do matter; what replaces it is §2.1.2's actual rule (aggregates only, never a row about an identified patient) plus §3's line that rooms and beds are an unbuilt domain rather than a missing tool, so the model says so instead of estimating. **The output guard takes the patient channel's treatment but not its copy**: a diagnosis assertion is replaced rather than annotated, because an administrator holds no clinical responsibility to exercise, and the replacement is operational copy rather than the patient's "please see a healthcare professional". Two smaller consequences fall out of the exhaustive channel maps the compiler forced open: **context enrichment returns nothing for an admin**, since every field §5.3 allows is about the asking user as a patient or a clinician, and **retrieval reads the `DOCTOR`-visibility corpus**, because that column is the store's name for "staff-only" rather than for "clinicians only".
11. `P15-T18` The five admin tools: `get_queue_board_summary` (allowlist drops `entries[]`), `get_daily_cashier_report` (`byDoctor` retained and documented), `get_appointment_load`, plus the two pharmacy tools shared with the doctor channel. Specs assert no patient-identifying field survives any projection, including one where `getQueueBoard` returns a full `entries[]` array and none of it appears. **No personal patient data in either mode**, so this ships without waiting on `P15-T16`. **Done.** `get_queue_board_summary` is the tool the §4.3 allowlist was designed for: `QueueBoardResponse` carries `entries[]` — one row per queued patient, by name and MRN — and the tool copies the counts and never the roster, so §2.1.2's rule is mechanical rather than remembered. The two pharmacy tools are **shared rather than duplicated**: they carry no personal data, so one projection answers both audiences instead of two that could drift, and expiry going live for admins was the one-line channel-list change §2.1.1 predicted rather than a permission edit. Two corrections to this section landed with the code and are recorded above: the `get_appointment_load` permission row, and the fact that the cashier service takes no `CurrentUser`. `byDoctor` ships included as specified, with `doctorId` dropped — the name is what makes the line readable and the id adds nothing the answer needs. Coverage: 20 unit cases across the three tools, 4 registrar cases pinning the admin catalogue (including that an admin holding every grant is still offered no patient tool, since both the scope rule and the channel list exclude them), and 4 end-to-end cases — the 403 for a non-admin opening the channel, the five-tool catalogue on the wire with the admin prompt rather than the clinician's, a queue answer with the named patient row absent from both the client response and the outbound body, and an admin exchange carrying no context-enrichment payload at all.
