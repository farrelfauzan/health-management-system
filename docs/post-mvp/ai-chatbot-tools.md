...
13. `P15-T10` `ClinicDocument` / `ClinicDocumentChunk` schema with channel visibility, language tag, `embeddingModel` / `embeddingVersion`, vector + `tsvector` columns; admin CRUD behind `clinic-document.write:any`; chunking and embedding on ingest via Ollama (`bge-m3`).
14. `P15-T11` Hybrid retrieval (§5.3): vector + full-text, fused with RRF, channel-filtered, prepended as a `SYSTEM` turn in the completion path, citations in the reply, answer-language-follows-question, non-fatal on miss.
