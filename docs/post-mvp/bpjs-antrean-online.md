...
5. `P14-T05` Outbound publishing: `add`/`panggil`/`batal` as new outbox types with the provenance skip; HFIS reconciliation screen. **Done** — on the same outbox table, worker and monitor as PCare (§4.4), with the provenance skip enforced at enqueue *and* in the submitter, and reconciliation shipped as a read-only drift report (§4.3). Field names remain the spike's hypotheses. See [implementation-plan.md](./implementation-plan.md) §9.
6. `P14-T06` UAT support: run the BPJS checklist against the deployed pilot, re-record fixtures from real responses, and write the onboarding runbook entry (branch-office request, public-IP requirement, credential intake, UAT walkthrough). **Prepared, not run** — the checklist, the onboarding runbook and a redacting protocol-capture instrument shipped; the run itself needs a pilot, credentials and a booked slot, none of which exist. §6's *"Fixtures test our reading of the spec, not the spec"* is why the fixture set is still empty. See [bpjs-antrean-uat.md](./bpjs-antrean-uat.md).
