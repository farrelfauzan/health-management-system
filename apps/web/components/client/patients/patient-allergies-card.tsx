...
export function PatientAllergiesCard({ allergies }: PatientAllergiesCardProps) {
  ...
  const orderedAllergies = [...allergies].sort(
    (left, right) =>
      ALLERGY_SEVERITY_ORDER.indexOf(left.severity) - ALLERGY_SEVERITY_ORDER.indexOf(right.severity),
  );
      ...
      <CardContent>
        {orderedAllergies.length > 0 ? (
          ...
          <p className="rounded-lg bg-slate-50 px-3 py-4 text-center text-sm text-slate-500">
            No known allergies recorded. Absence of a record is not the same as none reported.
          </p>
        )}
      </CardContent>
    </Card>
