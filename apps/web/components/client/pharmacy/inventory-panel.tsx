...
export function InventoryPanel() {
  ...
  const medicationParams: MedicationControllerListMedicationsV1Params = {
    ...
    ...(category !== 'ALL'
      ? { category: category as MedicationControllerListMedicationsV1Params['category'] }
      : {}),
  ...
  return (
    <div className="space-y-6">
      <InventoryStatCards
        ...
        expiringCount={expiryItems.filter((item) => item.expiryStatus === 'EXPIRING').length}
        isLoading={summaryQuery.isPending || expiryQuery.isPending}
        isError={summaryQuery.isError || expiryQuery.isError}
      />
      ...
      <Card className="gap-0 overflow-hidden rounded-xl border-slate-200 py-0 shadow-none">
        ...
        <CardContent className="space-y-4 p-5">
          ...
          {!medicationsQuery.isPending && !medicationsQuery.isError && medications.length === 0 ? (
            <p className="py-10 text-center text-sm text-slate-500">{t('emptyCatalog')}</p>
          ) : null}
                ...
                <TableBody>
                  {medications.map((medication) => (
                    <TableRow key={medication.id}>
                      <TableCell className="font-mono text-xs">{medication.code}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        {medication.category ? formatStatusLabel(medication.category, locale) : '-'}
                      </TableCell>
                      <TableCell
                        className={medication.needsReorder ? 'font-semibold text-danger' : ''}
                      >
                        ...
                        <div className="flex gap-1">
                          {canUpdateMedication ? (
                            <Button
                              ...
                              size="icon-sm"
                              variant="ghost"
                              aria-label={`${t('editMedication')} ${medication.name}`}
                          ...
                          {canReceiveStock ? (
                            <Button
                              ...
                              size="icon-sm"
                              variant="ghost"
                              aria-label={`${t('receiveStock')} ${medication.name}`}
                              onClick={() => setReceiptMedicationId(medication.id)}
                            >
                              <Icon name="add_box" size={17} />
                            </Button>
                          ) : null}
                        </div>
                      </TableCell>
          ...
          <NumberedPagination
            ...
            isDisabled={medicationsQuery.isFetching}
          />
        </CardContent>
      </Card>
      {expiryQuery.isError ? (
        <InlineNotice tone="error">{t('inventoryLoadError')}</InlineNotice>
      ) : (
        <ExpiryReportTable items={expiryItems} throughDate={expiryQuery.data?.throughDate} />
      )}
      ...
      {receiptMedicationId !== null ? (
        <ReceiveStockDialog
          key={receiptMedicationId || 'select'}
          open
          onOpenChange={(open) => {
            if (!open) setReceiptMedicationId(null);
          }}
          medications={medicationOptionsQuery.medications}
          initialMedicationId={receiptMedicationId || undefined}
          onSaved={showMessage}
        />
      ) : null}
    </div>
