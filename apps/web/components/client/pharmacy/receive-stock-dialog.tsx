        ...
        <form noValidate onSubmit={(event) => void handleSubmit(event)}>
          ...
          <div className="space-y-4 py-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5"><FormLabel htmlFor="receive-stock-batch-number" required>{t('batchNumber')}</FormLabel><Input id="receive-stock-batch-number" value={batchNumber} onChange={(event) => setBatchNumber(event.target.value)} /></div>
              ...
              <div className="space-y-1.5"><FormLabel htmlFor="receive-stock-quantity" required>{t('quantity')}</FormLabel><Input id="receive-stock-quantity" type="number" min="1" max="1000000" value={quantity} onChange={(event) => setQuantity(event.target.value)} /></div>
              <div className="space-y-1.5"><FormLabel htmlFor="receive-stock-received-at">{t('receivedAt')}</FormLabel><Input id="receive-stock-received-at" type="datetime-local" value={receivedAt} onChange={(event) => setReceivedAt(event.target.value)} /></div>
            </div>
            <div className="space-y-1.5"><FormLabel htmlFor="receive-stock-notes">{t('notes')}</FormLabel><Textarea id="receive-stock-notes" maxLength={1000} value={notes} onChange={(event) => setNotes(event.target.value)} /></div>
          </div>
          <DialogFooter>
