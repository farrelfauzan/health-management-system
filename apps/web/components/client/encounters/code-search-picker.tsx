...
export function CodeSearchPicker({
  ...
  if (selected) {
    return (
      <div>
        <span className="mb-1.5 block font-heading text-xs font-medium text-slate-600">{label}</span>
      ...
      {isEnabled ? (
        <div className="mt-1.5 max-h-56 overflow-y-auto rounded-lg border border-slate-200">
          {isPending ? (
            ...
            <p className="px-3 py-2 text-sm text-slate-500">No matching code.</p>
          ) : (
            <ul>
              {codes.map((option) => (
                <li key={option.id}>
                  ...
                  >
                    <span className="font-mono text-xs font-medium text-primary">{option.code}</span>
