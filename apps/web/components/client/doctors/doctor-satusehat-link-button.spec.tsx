...
const { linkDoctorMock, toastErrorMock, toastSuccessMock } = vi.hoisted(() => ({
  linkDoctorMock: vi.fn(),
...
const { DoctorSatusehatLinkButton } = await import('./doctor-satusehat-link-button');
...
describe('DoctorSatusehatLinkButton', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    linkDoctorMock.mockResolvedValue({ status: 200, data: {} });
  });

  it('offers the link when the doctor has a NIK and is not linked', () => {
    renderButton({});
  ...
  it('renders nothing once the doctor is linked — there is nothing left to do', () => {
  ...
  it.each([
    ...
    [409, 'Lebih dari satu kecocokan — verifikasi tenaga medis di portal SATUSEHAT dulu.'],
    ...
    linkDoctorMock.mockRejectedValue(buildAxiosErrorWithStatus(status));
    renderButton({});
