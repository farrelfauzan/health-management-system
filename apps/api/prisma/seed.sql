...
-- Open the MRN sequence for the default facility. The seed no longer creates
-- patients, so the counter simply starts at 1 and the first API-created patient
-- takes `00000001`; the row is still seeded rather than created on demand so
-- that allocation never has to race two concurrent registrations to insert it.
-- Idempotent and monotonic: re-running the seed never moves the counter
-- backwards, so a database that has issued numbers keeps its place.
...
-- Bootstrap operator accounts. Credentials:
--   admin@salingjaga.com    / Admin123!
--   pharmacy@salingjaga.com / Pharmacy123!
--
-- These two are the only interactive logins the seed creates, and they exist
-- because a fresh database is otherwise unreachable: every screen that creates
-- a practitioner, a patient or a tariff is itself behind the permission guard,
-- so something has to be able to sign in first. Doctor logins are NOT seeded --
-- a practitioner account is created through the admin screens together with the
-- `doctor_profiles` row it owns, which is the only path that can write an
-- encrypted practitioner NIK.
--
-- The hashes are bcryptjs (cost 10). Change both passwords, or remove this
-- block outright, before this seed is pointed at anything but development.
