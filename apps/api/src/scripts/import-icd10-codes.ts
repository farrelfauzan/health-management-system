 ...
 */
function readIcd10Rows(filePath: string): Icd10ImportRow[] {
...
/**
 * Replaces the ICD-10 catalog with an official list.
 *
 * Codes in the file are upserted and activated; codes absent from it are
 * deactivated rather than deleted, because historic diagnoses must keep
 * resolving the code they were signed with. The whole run is one transaction,
 * so a malformed file leaves the catalog untouched.
 */
async function importIcd10Codes(): Promise<void> {
  const filePath = process.argv[2];
  if (!filePath) {
    throw new Error('Usage: pnpm --filter @hms/api icd10:import <file.csv>');
  }
  const rows = readIcd10Rows(filePath);
  const pool = new Pool({
  ...
  try {
    ...
    const { rows: upserted } = await client.query<{ inserted: boolean }>(
      `INSERT INTO "icd10_codes" (
         "id", "code", "display", "display_indonesian", "category", "chapter",
         "is_active", "created_at", "updated_at", "deleted_at"
       )
       SELECT
         gen_random_uuid(), source.code, source.display, source.display_indonesian,
         split_part(source.code, '.', 1), source.chapter, true, NOW(), NOW(), NULL
      ...
      [
        ...
        rows.map((row) => deriveIcd10Chapter(row.code)),
      ],
    );
    ...
    console.log(
      `Imported ${rows.length} ICD-10 code(s): ${insertedCount} new, ${rows.length - insertedCount} updated, ${deactivatedCount ?? 0} deactivated.`,
    );
  } catch (err: unknown) {
...
importIcd10Codes().catch((err: unknown) => {
  console.error('ICD-10 import failed:', err instanceof Error ? err.message : err);
  process.exit(1);
});
