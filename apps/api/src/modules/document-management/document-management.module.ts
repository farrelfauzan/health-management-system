 ...
 * consumer of `ObjectStorageService`. This one completes `P15-T10` with the
 ...
 * The owner-scoped personal routes (`P15-T20`) remain; the repository already
 * takes `ownerType`/`ownerId` as required arguments, so they are not a
 * widening of any query written here.
 *
...
@Module({
  ...
  exports: [DocumentRepository, DocumentChunkRepository, DocumentService],
})
