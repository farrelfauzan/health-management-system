import {
  AiProviderCircuitBreakerOptions,
  AiProviderCircuitBreakerState,
} from './ai-provider.types';

/**
 * Minimal circuit breaker, one instance per provider config (same policy as
 * the SATUSEHAT and BPJS breakers, but keyed per `configId` by the caller —
 * one clinic's bad upstream must not trip another's). Consecutive transport
 * failures open the circuit; after the open duration one half-open probe is
 * allowed through, and its outcome either closes or re-opens the circuit.
 * Business-level rejections (4xx) must be recorded as successes by the
 * caller — they prove the upstream is reachable, which is all this class
 * measures.
 */
export class AiProviderCircuitBreaker {
  private state: AiProviderCircuitBreakerState = 'CLOSED';
  private consecutiveFailureCount = 0;
  private openedAtEpochMs = 0;
  private hasHalfOpenProbeInFlight = false;
  private readonly resolveNow: () => number;

  constructor(private readonly options: AiProviderCircuitBreakerOptions) {
