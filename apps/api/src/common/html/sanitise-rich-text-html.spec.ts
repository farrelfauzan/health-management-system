import { sanitiseRichTextHtml } from './sanitise-rich-text-html';
 ...
 */
describe('sanitiseRichTextHtml', () => {
  describe('strips the injection routes NFR-SEC-01 names', () => {
    it('drops a script tag and its body', () => {
      const actual = sanitiseRichTextHtml('<p>Total</p><script>alert(1)</script>');
    ...
    it('drops inline event handlers', () => {
      const actual = sanitiseRichTextHtml('<img src="x" onerror="alert(1)"><p onclick="p()">a</p>');
    ...
    it('drops javascript: URLs with the anchor around them', () => {
      const actual = sanitiseRichTextHtml('<a href="javascript:alert(1)">pay here</a>');

      expect(actual).toBe('pay here');
    });

    it('drops iframe, object, embed, and link elements', () => {
      const actual = sanitiseRichTextHtml(
    ...
    it('drops a style element outright', () => {
      const actual = sanitiseRichTextHtml(
        '<style>@import url(https://evil.example);</style><p>a</p>',
      );
    ...
    it('drops a remote image source but keeps an inline one', () => {
      const remote = sanitiseRichTextHtml('<img src="https://evil.example/x.png">');
      const relative = sanitiseRichTextHtml('<img src="/logo.png">');
      const inline = sanitiseRichTextHtml('<img src="data:image/png;base64,iVBORw0KGgo=">');
      const dataDocument = sanitiseRichTextHtml('<img src="data:text/html;base64,PHNjcmlwdD4=">');
  ...
  describe('keeps what an invoice layout is made of', () => {
    it('keeps table structure with span attributes', () => {
      ...
      expect(sanitiseRichTextHtml(inputHtml)).toBe(inputHtml);
    });
  ...
  describe('canonicalises variable tokens', () => {
    ...
    it('keeps a block token on a div', () => {
      const actual = sanitiseRichTextHtml('<div data-hms-var="items"><p>preview rows</p></div>');

      expect(actual).toBe('<div data-hms-var="items"></div>');
    });

    it('drops a token attribute whose value is not token-shaped', () => {
      const actual = sanitiseRichTextHtml('<span data-hms-var="\'; DROP TABLE--">x</span>');
  ...
  it('is idempotent over its own output', () => {
    ...
    const oncePass = sanitiseRichTextHtml(hostileInput);

    expect(sanitiseRichTextHtml(oncePass)).toBe(oncePass);
  });
});
